...
// The synthesis lives in sound.js; the harness runs it ahead of the game, the
// way the page loads it.
...
test('every voice is a no-op rather than a throw when audio is unavailable', () => {
  ...
  g.Sound.fire(50);
  for (let seat = 0; seat < 6; seat++) g.Sound.fire(80, seat, seat / 5); // every reload signature
  g.Sound.explode(26);
...
test('sound defaults to on', () => {
  ...
  assert.equal(g.SOUND.enabled, true, 'sound.js loaded and published its global');
  assert.ok(g.SOUND.volume > 0 && g.SOUND.volume <= 1, 'master volume is a 0..1 fraction');
  assert.equal(g.Sound.muted(), false);
});

test('the sound slider in settings is labelled in both languages', () => {
  const { g } = loadFlat();

  for (const key of ['soundFx', 'sfxVolume']) {
    const en = g.txt(key);
  ...
  assert.equal(g.txt('soundFx'), 'SES EFEKTLERİ', 'and a Turkish one');
...
test('sounds are placed where things are on screen, so the camera moves them', () => {
  const { g } = loadFlat();

  g.camFit();
  assert.equal(g.screenX01(g.SIM_W / 2), 0.5, 'zoomed out, the middle of the world is centred');
  assert.equal(g.screenX01(0), 0);

  g.camZoomTo(2, 0, 0.5); // zoom in about the left edge
  assert.equal(g.screenX01(0), 0, 'the left edge stays put');
  assert.equal(g.screenX01(g.SIM_W / 2), 1, 'and the middle of the world is now the right edge');
});
